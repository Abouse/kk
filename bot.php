	...
	$callback = function ($update, $bot) {
      ...
      $accounts = json_decode(file_get_contents('accounts.json'),1);
			if(isset($update->message)){
              ...
              $bot->sendMessage([
                  'chat_id'=>$chatId,
                  'text'=>"اهلا بك . \n - اداه فحص حسابات الانستاقرام. \n\n By :-@B_Q_X",
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
                          [['text'=>'اضافه حساب🛂','callback_data'=>'login']],
                          [['text'=>'اعدادت الفحص🔎','callback_data'=>'grabber']],
                          [['text'=>'تشغيل فحص✅','callback_data'=>'run'],['text'=>'اايقاف البحث❌','callback_data'=>'stop']],
                          [['text'=>'حالة فحص⚠️','callback_data'=>'status']],
                          [[ 'text'=>'CH @DEV_Tools ','url'=>'t.me/DEV_T0Ols']],
                      ]
                  ])
              ]);   
          } elseif($text != null){
          	if($config['mode'] != null){
          		...
          		if($mode == 'addL'){
		              ...
		              $keyboard['inline_keyboard'][] = [['text'=>'الرجوع الي صفحه الرئسيه','callback_data'=>'back']];
		              $bot->editMessageText([
		                  'chat_id'=>$chatId,
		                  'message_id'=>$mid,
                  ...
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
                          [['text'=>'اضافه حساب🛂','callback_data'=>'login']],
                          [['text'=>'اعدادت الفحص🔎','callback_data'=>'grabber']],
                          [['text'=>'تشغيل فحص✅','callback_data'=>'run'],['text'=>'ايقاف البحث❌','callback_data'=>'stop']],
                          [['text'=>'حالة فحص⚠️','callback_data'=>'status']],
                          [[ 'text'=>'CH @DEV_Tools ','url'=>'t.me/DEV_T0Ols']],
                      ]
                  ])
					...
					$bot->sendMessage([
							'chat_id'=>$chatId,
							'text'=>"👻|| نسخه صيد متاحات انستكرام خاصه
لشراء ملفات النسخه او تنصيب راسلنا . ",
							'reply_markup'=>json_encode([
                  'inline_keyboard'=>[
                      [['text'=>'By ; 𝐴𝐻𝑀𝐸𝐷 𝐷𝐸𝑉𖤛','url'=>'t.me/B_Q_X']]
                  ]
							])
					]);
				}
          ...
          if($data == 'login'){
        		...
        		$keyboard = ['inline_keyboard'=>[
		              ...
		              $keyboard['inline_keyboard'][] = [['text'=>'الرجوع الي صفحه الرئيسيه','callback_data'=>'back']];
		              $bot->editMessageText([
		                  'chat_id'=>$chatId,
		                  'message_id'=>$mid,
		                  'text'=>"صفحة التحكم في الحسابات.",
		                  'reply_markup'=>json_encode($keyboard)
		              ]);
          } elseif($data == 'addL'){
            ...
            $bot->editMessageText([
                ...
                'reply_markup'=>json_encode([
                    'inline_keyboard'=>[
                        ...
                        [['text'=>'من الهاشتاكات #','callback_data'=>'hashtag'],['text'=>'من اكسبلور .','callback_data'=>'explore']],
            ...
            $bot->sendMessage([
                'chat_id'=>$chatId,
                'text'=>"ارسل الكلمات للبحث \n - مثال saw Ahmed dev Egpyt photo"
            ]);
            $config['mode'] = 'search';
            file_put_contents('config.json', json_encode($config));
            ...
            exec('screen -dmS gr php explore.php');
          	...
          	if($data[0] == 'del'){
          		...
          		file_put_contents('accounts.json', json_encode($accounts));
              $keyboard = ['inline_keyboard'=>[
							[['text'=>"إضافة حسابات جديدة",'callback_data'=>'addL']]
									]];
		              foreach ($accounts as $account => $v) {
		                  $keyboard['inline_keyboard'][] = [['text'=>$account,'callback_data'=>'ddd'],['text'=>"تسجيل الخروج",'callback_data'=>'del&'.$account]];
		              }
		              $keyboard['inline_keyboard'][] = [['text'=>'الرجوع الي صفحة الرئيسيه','callback_data'=>'back']];
		              $bot->editMessageText([
		                  'chat_id'=>$chatId,
		                  'message_id'=>$mid,
		                  'text'=>"صفحة التحكم في الحسابات.",
